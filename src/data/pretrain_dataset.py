...
class PretrainDataset(Dataset):
    ...
    def _generate_smart_mask(self, sequence: torch.Tensor) -> torch.Tensor:
        ...
        # Use torch.rand() instead of np.random.random() for consistency and speed
        ...
        if current_masked_count < target_masked_count:
            ...
            # Get indices that are not yet masked using torch operations
            # nonzero returns tuple of tensors, we take the first one
            ...
            if unmasked_indices.size(0) > 0:
                ...
                perm = torch.randperm(unmasked_indices.size(0))[:needed]
    ...
    def _volatility_aware_mask(
        ...
        if high_vol_indices.size(0) > 0:
            # Pick a random index from high_vol_indices
            idx_in_indices = torch.randint(0, high_vol_indices.size(0), (1,)).item()
    ...
    def _cross_asset_mask(
        ...
        for feat_idx in self.price_feature_indices:
            if torch.rand(1).item() < 0.15:
                ...
                # Use torch.randperm for sampling
