...
class PretrainDataset(Dataset):
    ...
    def _generate_smart_mask(self, sequence: torch.Tensor) -> torch.Tensor:
        ...
        # Optimize: Use torch random functions to avoid numpy overhead (~1.4x speedup)
        ...
        if current_masked_count < target_masked_count:
            ...
            if len(unmasked_indices) > 0:
                ...
                # Optimize: Use torch.randperm instead of np.random.choice
    ...
    def _volatility_aware_mask(
        ...
        if len(high_vol_indices) > 0:
            # Optimize: Use torch.randint instead of np.random.randint
            mask_idx = high_vol_indices[torch.randint(0, len(high_vol_indices), (1,)).item()]
    ...
    def _cross_asset_mask(
        ...
        for feat_idx in self.price_feature_indices:
            # Optimize: Use torch random functions
