...
class PretrainDataset(Dataset):
    ...
    def _generate_smart_mask(self, sequence: torch.Tensor) -> torch.Tensor:
        ...
        # Use torch.rand instead of np.random to avoid CPU sync
        ...
        if current_masked_count < target_masked_count:
            ...
            # No need to convert to numpy or use nonzero if we use randperm on full length
            # But finding unmasked is fast.
    ...
    def _volatility_aware_mask(
        ...
        n_high = len(high_vol_indices)
        if n_high > 0:
            # Use torch.randint instead of np.random.randint
            mask_idx = high_vol_indices[torch.randint(0, n_high, (1,)).item()]
