...
# Defaults; keep in sync with Janus dataset builder
INPUT_WINDOW_CANDLES = 27
DATA_PATH_DEFAULT = os.path.join(os.path.dirname(__file__), "..", "Dataset", "janus_m15_dataset.parquet")


def _chronological_split(df: pd.DataFrame, cutoff: str = "2025-01-01") -> Tuple[pd.DataFrame, pd.DataFrame]:
...
class JanusTimeseriesDataset(Dataset):
...
def create_dataloaders(
    ...
    Train/Val: rows before 2025-01-01 (90/10 split)
    Test: rows on/after 2025-01-01
    ...
    # Chronological split
    train_val_df, test_df = _chronological_split(df, cutoff="2025-01-01")

    # Build full datasets
    full_train_val = JanusTimeseriesDataset(df=train_val_df, seq_len=seq_len)
    test_dataset = JanusTimeseriesDataset(df=test_df, seq_len=seq_len)
