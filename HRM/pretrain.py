...
def _compute_metrics(y_true: torch.Tensor, y_pred: torch.Tensor, num_classes: int = 5) -> Dict[str, float]:
    # y_true, y_pred: [N]
    ...
    # Add key minority class metrics (0 strong_sell, 4 strong_buy)
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    with torch.inference_mode():
        for X, y in loader:
            ...
            ps.append(preds.detach().cpu())

    y_all = torch.cat(ys) if ys else torch.empty(0, dtype=torch.long)
    p_all = torch.cat(ps) if ps else torch.empty(0, dtype=torch.long)
    base = {"val/loss": total_loss / max(1, total_count)}
    if y_all.numel() > 0:
        base.update(_compute_metrics(y_all, p_all, num_classes=5))
    return base
